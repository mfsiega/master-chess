...
export class Feedback extends Component {
  ...
  render() {
    ...
    if (this.state.wrongMoveEvaluation) {
      cpl =
        this.state.wrongMoveEvaluation.playthroughEval.score -
        this.state.wrongMoveEvaluation.referenceEval.score;
      cpl = cpl.toFixed(1);
    }
    return (
      <div
      ...
      >
        ...
        <div
          ...
          style={{
            display:
              this.props.lastPlayerMove && !this.props.gameOver
                ? "block"
                : "none",
          }}
        >
          You played {this.props.lastPlayerMove}{" "}
          {this.props.handlingWrongMove
            ? ` - they played ${this.props.handlingWrongMove.referencePlayed}`
            : " - correct!"}
          <br />
          {this.props.handlingWrongMove
            ? this.state.wrongMoveEvaluation
              ? `Eval diff: ${cpl}`
              : "Evaluation..."
            : ""}
        </div>
